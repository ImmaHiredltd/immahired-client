...
import { useGetPackageStatusQuery, useGetTalentMutation, useJobViewsQuery, useSaveJobMutation, useUnsaveJobMutation } from '@/app/api/general';
...
export default function JobDetails() {
  ...
  const { data: jobViewsData, isLoading: jobViewsLoading } = useJobViewsQuery(jobDetails?.id);
  ...
  useEffect(() => {
    ...
    if (emp) {
      setCompanyImage(JSON.parse(emp).logo.url || ppp)
        ...
        <div className='w-[90%] sm:w-[60%] bg-gradient-to-br from-white to-gray-200 relative p-5 rounded shadow-md shadow-black max-h-[80vh] overflow-y-scroll'>
          ...
          <div className='space-y-5'>
            <div className='flex items-center gap-3 max-sm:justify-between mt-3'>
              ...
              <span className='text-sm sm:text-[2rem] font-extrabold'>{empData?.companyName}</span>
            </div>

            <div className='flex flex-col gap-3'>
              <span className='font-semibold'>Company Overview:</span>
              <span className='text-sm'>{empData?.overview}</span>
            </div>
            ...
            <div className='flex flex-col gap-3'>
              ...
              <span className='text-sm'>{empData.contactInformation && parse(formatTextWithLineBreaks(empData?.contactInformation))}</span>
            </div>

            <div className='flex flex-col gap-3'>
              <span className='font-semibold'>Equal Opportunity Statement:</span>
              <span className='text-sm'>{empData.equalOpportunityStatement}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <ToastContainer />
      <section className="px-banner-clamp sm:py-32 py-28 space-y-16 bg-gray-50">
