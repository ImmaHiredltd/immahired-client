...
export default function WhyUs({ target }: any) {
  ...
  const Why = ({ icon, title, details }: any) => {
    return (
      <motion.div
        className="xl:w-full w-full btn-sweep py-7 sm:py-8 px-5 sm:px-8 rounded-xl bg-gray-200 space-y-5 ease-in-out duration-500 hover:bg-main hover:text-white"

        viewport={{ once: true, amount: 0.3 }}
      >
        <p className="text-2xl max-[380px]:text-xl max-[380px]:p-2 sm:text-4xl p-3 w-16 sm:w-20 h-16 sm:h-20 flex items-center justify-center text-white bg-main border-dotted border-2 border-gray-200  rounded-full">
          {icon}
        </p>

        <p className="text-xl max-[380px]:text-sm sm:text-2xl font-black">{title}</p>

        <p className='max-[380px]:text-xs w-[95%]'>{details}</p>
      </motion.div>
    );
  };
